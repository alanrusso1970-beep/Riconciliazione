...
STATION_CACHE = SimpleCache(ttl=600)  # 10 minuti
HISTORY_CACHE = SimpleCache(ttl=300)  # 5 minuti
...
class ProxyHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_proxy(self, method):
        ...
        if qs.get('action') == ['get_station_csv']:
            ...
            try:
                csv_data = STATION_CACHE.get()
                if not csv_data:
                    cb = os.urandom(4).hex()
                    csv_url = STATION_SHEETS_CSV_URL + f"&_cb={cb}"
                    req = urllib.request.Request(csv_url, headers={'User-Agent': 'Mozilla/5.0'})
                    with urllib.request.urlopen(req, context=ssl_context, timeout=30) as resp:
                        csv_data = resp.read().decode('utf-8-sig', errors='replace') # utf-8-sig gestisce il BOM
                    STATION_CACHE.set(csv_data)
                    print("[Proxy] Station CSV downloaded and cached")
                else:
                    print("[Proxy] Station CSV loaded from Cache")
                ...
                def normalize_pbl(p):
                    p_str = str(p).strip()
                    p_stripped = p_str.lstrip('0')
                    return p_stripped if p_stripped else p_str

                pbl_norm = normalize_pbl(pbl)
                ...
                for row in rows[1:]:
                    ...
                    if normalize_pbl(row_pbl) == pbl_norm:
                        station_data = {
                            'pbl': row_pbl,
        ...
        if qs.get('action') == ['get_history_csv']:
            pbl = (qs.get('pbl') or [''])[0].strip()
            print(f"[Proxy] Fetching History CSV via GAS (PBL={pbl})")
            try:
                csv_data = HISTORY_CACHE.get()
                if not csv_data:
                    target_url = f"{GAS_URL}?action=get_history_csv"
                    req = urllib.request.Request(target_url, headers={'User-Agent': 'Mozilla/5.0'})
                    with urllib.request.urlopen(req, context=ssl_context, timeout=30) as resp:
                        csv_data = resp.read().decode('utf-8', errors='ignore')
                    HISTORY_CACHE.set(csv_data)
                    print("[Proxy] History CSV downloaded and cached")
                else:
                    print("[Proxy] History CSV loaded from Cache")
                ...
                if pbl:
                    ...
                    filtered_csv = f_out.getvalue()
                    print(f"[Proxy] Filtered history: {match_count} rows for PBL={pbl}")
                    result_data = filtered_csv.encode('utf-8')
                else:
                    result_data = csv_data.encode('utf-8')
        ...
        try:
            ...
            with urllib.request.urlopen(req, context=ssl_context, timeout=30) as resp:
                ...
                # Invalida cache dello storico in caso di successo POST
                try:
                    resp_json = json.loads(result_data.decode('utf-8'))
                    if resp_json.get('success'):
                        print("[Proxy] POST success. Invalidating History Cache.")
                        HISTORY_CACHE.invalidate()
                except Exception as cache_err:
                    print(f"[Proxy] Warning: Could not decode response to invalidate cache: {str(cache_err)}")
                    # Comunque invalidiamo per sicurezza
                    HISTORY_CACHE.invalidate()
