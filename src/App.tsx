...
export default function App() {
  ...
  const loadStation = async () => {
    ...
    try {
      ...
      try {
        const histCsvUrl = `${baseUrl}/api/?action=get_history_csv&pbl=${encodeURIComponent(currentPbl)}`;
                    ...
                    {row === 'totale' ? (
                      ...
                      <>
                        <input 
                          type="number" 
                          disabled={!isEditable}
                          className={`w-full h-6 px-1 text-right border-none bg-transparent focus:bg-blue-50 outline-none no-print ${!isEditable ? 'opacity-60 cursor-not-allowed' : ''} ${row === 'chiusura' && item.chiusura < item.apertura ? 'text-red-700 font-black' : ''}`}
            ...
            {cisterne.map((c, i) => (
              <tr key={i}>
                ...
                <td className="border p-0">
                  <input type="number" disabled={!isEditable} className={`w-full h-6 px-1 text-right border-none bg-transparent no-print font-medium ${!isEditable ? 'opacity-60 cursor-not-allowed' : ''}`} value={c.altezza} onChange={e => updateFuel(f, 'cisterne', i, 'altezza', parseInt(e.target.value) || 0)} />
                  <span className="hidden-screen print-show text-right pr-1 font-mono">{formatNum(c.altezza)}</span>
                </td>
                <td className="border p-0">
                  <input type="number" disabled={!isEditable} className={`w-full h-6 px-1 text-right border-none bg-transparent no-print font-bold ${!isEditable ? 'opacity-60 cursor-not-allowed' : ''}`} value={c.giacenza} onChange={e => updateFuel(f, 'cisterne', i, 'giacenza', parseInt(e.target.value) || 0)} />
                    ...
                    <td key={idx} className={`border px-1.5 py-1 text-right ${cell.red ? 'text-red-600' : ''} ${shouldHideInPrint(fuels[idx]) ? 'print:hidden' : ''}`}>
                      <div className="flex justify-end items-center gap-1">
                        {cell.k && (
                          <input type="number" 
                            disabled={!isEditable}
                            className={`w-16 h-5 text-right border border-gray-200 rounded px-1 no-print bg-white focus:ring-1 focus:ring-blue-500 outline-none ${!isEditable ? 'opacity-60 cursor-not-allowed' : ''}`}
                            value={cell.v} 
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 print:p-0 print:bg-white font-sans text-slate-900 leading-relaxed">
      ...
      <div className={`max-w-7xl mx-auto space-y-5 ${showUTFForm ? 'print:hidden' : ''}`}>
        ...
        {/* ANAGRAFICA */}
        <div className={`bg-white p-6 rounded-3xl border shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-shadow duration-300 space-y-4 animate-fade-in-up delay-100 ${!isEditable && hasLoadedData ? 'border-amber-200 bg-amber-50/30' : 'border-slate-100'}`}>
          {!isEditable && hasLoadedData && (
            <div className="no-print flex items-center gap-2 bg-amber-100 text-amber-800 text-[11px] font-bold px-4 py-2 rounded-xl border border-amber-200">
              <Lock size={14} />
              Dati in sola lettura — Clicca <span className="text-emerald-700 uppercase">NUOVO</span> per modificare
            </div>
          )}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="flex flex-col">
              ...
              <input className={`border-b outline-none no-print font-medium ${!isEditable && hasLoadedData ? 'opacity-60 cursor-not-allowed' : ''}`} disabled={!isEditable && hasLoadedData} value={data.station.gestore} onChange={e => updateStation('gestore', e.target.value)} />
            ...
            <div className="flex flex-col">
              ...
              <input className={`border-b outline-none no-print font-medium ${!isEditable && hasLoadedData ? 'opacity-60 cursor-not-allowed' : ''}`} disabled={!isEditable && hasLoadedData} value={data.station.indirizzo} onChange={e => updateStation('indirizzo', e.target.value)} />
            ...
            <div className="flex flex-col">
              ...
              <div className="flex gap-1">
                <input placeholder="Comune" className={`w-1/2 border-b outline-none no-print font-medium ${!isEditable && hasLoadedData ? 'opacity-60 cursor-not-allowed' : ''}`} disabled={!isEditable && hasLoadedData} value={data.station.comune} onChange={e => updateStation('comune', e.target.value)} />
                <input placeholder="Località" className={`w-1/2 border-b outline-none no-print font-medium ${!isEditable && hasLoadedData ? 'opacity-60 cursor-not-allowed' : ''}`} disabled={!isEditable && hasLoadedData} value={data.station.localita} onChange={e => updateStation('localita', e.target.value)} />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="flex flex-col">
              ...
              <input className={`border-b outline-none no-print font-medium ${!isEditable && hasLoadedData ? 'opacity-60 cursor-not-allowed' : ''}`} disabled={!isEditable && hasLoadedData} value={data.station.marchio} onChange={e => updateStation('marchio', e.target.value)} />
            ...
            <div className="flex flex-col md:col-span-2">
              ...
              <div className="flex gap-2 items-center">
                <input type="date" className={`border-b no-print text-[10px] outline-none ${!isEditable && hasLoadedData ? 'opacity-60 cursor-not-allowed' : ''}`} disabled={!isEditable && hasLoadedData} value={data.station.data} onChange={e => updateStation('data', e.target.value)} />
