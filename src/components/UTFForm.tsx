...
export default function UTFForm({ data, onClose }: UTFFormProps) {
  ...
  const getInitialTableData = (startLetter: string, empty: boolean = false) => {
    ...
    rowIds.forEach(rowId => {
      ...
      fuels.forEach(f => {
        ...
        initialValues[rowId][f] = f === 'olio' ? Math.round(val * 100) / 100 : Math.round(val);
      });
    });
  ...
  const handleChangeTable = (table: 1 | 2, rowId: string, fuel: string, value: string) => {
    const num = fuel === 'olio' ? (parseFloat(value) || 0) : (parseInt(value) || 0);
    if (table === 1) {
  ...
  const renderTableRows = (tableNum: 1 | 2) => {
    ...
    return rowLabels.map((row) => (
      <tr key={row.id}>
        ...
        {fuels.map((f) => {
          const cellVal = currentTableData[row.id][f];
          const isOlio = f === 'olio';
          const displayVal = isOlio && cellVal !== 0 ? cellVal.toFixed(2) : (cellVal === 0 ? '' : cellVal);
          return (
            <td key={f} className="border border-black p-0 text-right font-black text-[10px]">
              <input 
                ...
                value={cellVal === 0 ? '' : (isOlio ? cellVal : cellVal)} 
    ...
    <div className="fixed inset-0 bg-white z-[500] overflow-y-auto p-2 md:p-8 font-serif text-black print:static print:p-0 print:overflow-visible">
      ...
      <div className="max-w-[1100px] mx-auto bg-white p-2 print:p-0 print:max-w-full text-black">
        ...
        <div className="text-[11px] leading-relaxed mb-3 space-y-1.5 px-1">
          ...
          <div className="flex flex-wrap items-end gap-x-1">
            <div className="border-b border-black min-w-[100px] relative h-4">
              <input className="absolute inset-0 w-full bg-transparent border-none outline-none text-center font-black no-print" value={extra.delData} onChange={e => handleChangeExtra('delData', e.target.value)} />
              <span className="hidden print:block text-center font-black">{extra.delData}</span>
            </div>
        ...
        <div className="mb-2 px-1 text-black">
          ...
          <div className="flex flex-row gap-1.5 w-full">
            {['spb', 'gasolio', 'supreme'].map((f, i) => {
              const dispensers = (data.fuels[f]?.dispensers || []).filter((d: any) => d.chiusura > 0);
              const label = ['BENZINA', 'GASOLIO', 'GASOLIO SUPREME'][i];
              return (
        ...
        <div className="flex flex-col mt-2 px-1">
          ...
          <div className="w-full flex flex-col items-end pt-2 no-print">
            <div className="border-t-2 border-black w-1/3 pt-2 font-black uppercase text-[11px] tracking-tight text-center">
